...
public class TableNoFrontDB extends SQLiteOpenHelper {
    ...
    private static String DB_NAME="frontTables";
  ...
  public static String DB_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + "/databases/";
    ...
    public void deleteProduct(String selectedName, int selectedId) {
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("DELETE FROM " + TABLE_PRODUCTS + " WHERE " + COLUMN_ID + "=\"" + selectedId + "\";");
    }
    public void deleteProductByName(String selectedName) {
        SQLiteDatabase db = getWritableDatabase();
...
public String[] getTables(){
        ...
        SQLiteDatabase db = getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT " + COLUMN_TABLENUM + " FROM "+TABLE_PRODUCTS, null);
        String[] array = new String[cursor.getCount()];
        int i = 0;
        while(cursor.moveToNext()){
            String tableNo = cursor.getString(cursor.getColumnIndex("tableNum"));
            array[i] = tableNo;
            i++;
        }
        return array;
